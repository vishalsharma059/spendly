...
function ExpenseModal({ open, onClose, editingExpense }: ExpenseModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center p-3 sm:items-center sm:p-4" style={{ background: 'rgba(0,0,0,0.8)' }}>
      <div className="max-h-[calc(100dvh-1.5rem)] w-full max-w-lg overflow-y-auto rounded-2xl p-4 space-y-5 sm:max-h-[calc(100dvh-2rem)] sm:p-6" style={{ background: 'rgb(15,15,18)', border: '1px solid rgb(38,38,50)' }}>
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 pb-6 sm:px-6 lg:p-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="font-syne text-2xl font-bold text-white sm:text-3xl">Expenses</h1>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'rgb(15,15,18)', border: '1px solid rgb(28,28,35)' }}>
        {loading ? (
          ...
          <>
            <div className="hidden grid-cols-12 gap-4 px-5 py-3 border-b text-xs font-medium text-zinc-600 uppercase tracking-wider lg:grid"
            ...
            {filtered.map((exp) => (
              <div key={exp._id}
                ...
                <div className="text-xs text-zinc-500 lg:col-span-1">{formatDate(exp.date)}</div>
                <div className="min-w-0 lg:col-span-4">
                  <span className="block truncate text-sm font-medium text-zinc-200">{exp.description}</span>
                </div>
                <div className="min-w-0 lg:col-span-3">
                  {exp.categoryId ? (
                    <span className="inline-flex max-w-full items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-medium"
